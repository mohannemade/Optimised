...
const unsigned long interval = 1500;  //ms  // How often to send data to the other unit
...
void loop() {
  ...
  while ( network.available() ) {     // Is there any incoming data?
    ...
    network.read(header4, &msg, sizeof(msg)); // Read the incoming data
    Serial.println(msg);
  ...
  if (now - last_sent >= interval) {   // If it's time to send a data, send it!
    ...
    {
    //Serial.println(val);
    bool ok = network.write(header5, &val, sizeof(val)); // Send the data
    if(ok)
    {
